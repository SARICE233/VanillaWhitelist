...
> ⚠️ **注意：本文件是项目早期的设计稿，部分内容已被取代。**
>
> - **通信协议**（消息格式、字段、错误码）已抽出为三端共用契约，**以仓库根目录的 `PROTOCOL.md` 为准**。
>   本文件里的协议章节可能已过时（例如错误码 `PLAYER_NOT_FOUND` 已统一为 `NOT_WHITELISTED`）。
> - **实现细节**（线程模型、异步调度、SQLite 表结构等）仍可参考。
> - 面向使用者的安装与配置说明见 `README.md`。
